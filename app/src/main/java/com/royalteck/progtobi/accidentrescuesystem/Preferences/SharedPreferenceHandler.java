package com.royalteck.progtobi.accidentrescuesystem.Preferences;
...
public class SharedPreferenceHandler {
    ...
    public static void saveHospitalRecords(Context context, AccidentResponse hospitalResponse) {
        Gson gson = new Gson();
        String devRecords = gson.toJson(hospitalResponse);
        saveSharedPreferenceString(context, DEV_KEY, devRecords);
    }

    public static AccidentResponse fetchHospitalRecords(Context context) {
        SharedPreferences settings = context.getSharedPreferences(PREF_FILE, 0);
        String records = settings.getString(DEV_KEY, "{}");
        return new Gson().fromJson(records, AccidentResponse.class);
    }
